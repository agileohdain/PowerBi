...
- **`bg.png`** — fond visuel exporté depuis `powerpoint/Maquette Power BI.pptx`
  (via `powerpoint/export-bg.ps1`). Contient le bandeau, la zone logo, le fond.
- **`logo.png`** — logo du client (déjà intégré au `.pptx`/`bg.png` par
  l'utilisateur ; ce fichier est optionnel pour le rendu HTML si le logo est
  dans l'image de fond).

Si `bg.png` est absent → appeler `powerpoint/export-bg.ps1 -Path
clients/<client>/fond.pptx -Output clients/<client>/bg.png` (ou `-Template` pour
générer depuis le master avec les couleurs de `CLIENT.md`). Si `CLIENT.md` est
...
- `references/POWERBI_LAYOUT.md` — grille canevas, zones (le bandeau = image
  `bg.png`, ne PAS le redessiner en CSS), filtre pane, navigation deux-niveaux,
...
2. Vérifier `CLIENT.md` présent. Si non → stop, demander de créer depuis
   `clients/_template/`.
3. Vérifier `bg.png` présent. Si non :
...
2. Produire `clients/<client>/maquette/index.html` — fichier **auto-suffisant** :
   ...
   - **Fond = `bg.png`** appliqué sur le canevas
     (`background: url(./bg.png) center/cover no-repeat`). **Ne pas redessiner
     le bandeau ni la zone logo en CSS** — ils sont dans l'image.
   - Variables CSS en `:root` lues depuis `CLIENT.md` :
     `--primary`, `--surface`, `--canvas`, `--border`, `--card-bg`,
     `--text-primary`, `--text-secondary`.
   - **Titre + sous-titre** positionnés par-dessus le bandeau de `bg.png`
     (zone haute, ~0-97px), en `var(--surface)`.
   - Pane filtres à gauche avec **"Filtres" + icône en `var(--primary)`**.
     (L'arrière-plan du pane vient de `bg.png` ; ne poser qu'un fond
...
3. Copier `bg.png` (+ `logo.png` si présent) à côté de `index.html` dans
...
- **Ne pas redessiner le bandeau / la zone logo / le fond en CSS** : ils sont
  dans `bg.png`. Le skill ne pose que le titre, les slicers, les KPIs, les charts.
